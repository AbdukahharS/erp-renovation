/**
 * Default renovation template seeded into every new tenant.
 ...
 *
 * Edits to a tenant's copy of this template are independent — this constant
 * only defines the *initial* state at provisioning time.
 */
...
const photo = (description: string, required = true): SeedMediaRequirement => ({
	mediaType: "PHOTO",
	required,
	description,
});

const video = (description: string, required = false): SeedMediaRequirement => ({
	mediaType: "VIDEO",
	required,
	description,
});
...
export const DEFAULT_TEMPLATE: SeedTemplate = {
