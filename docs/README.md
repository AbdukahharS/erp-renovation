...
| A2 | Auto-estimation | **Partially superseded.** Auto-estimation from templates is still deferred (Owner fixes planned unit cost manually, $/m²), but a tenant-scoped **warehouse + materials catalog + issuance ledger** has shipped (see Phase 7 §7.2.1). Actual material cost on a property now comes from warehouse issuances, not free-form cost entry. | Per-stage quantity formulas / auto-estimation is its own module, off the critical path. Manual cost entry stays available for off-warehouse expenses. | Implementing per-stage estimation against the existing materials catalog. |
