...
### Notifications — Telegram Bot + Web Push
Telegram Bot is explicitly called out in the TZ and is genuinely the right channel for on-site construction workers — no app install required, reliable delivery, works on any phone. `grammy` is the Bun-compatible Telegram bot library. Web Push covers the browser/desktop side for inspectors and managers.
...
│   │   │   │   ├── notifications/  # Telegram bot, web push dispatch
